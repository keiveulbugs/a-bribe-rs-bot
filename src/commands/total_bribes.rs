...
pub async fn total_bribes(
    ...
    let firstmessage = ctx
        .send(|b| {
            { b.embed(|c| c.description("Starting to look up the contract")) }.ephemeral(true)
        })
        .await?;
    ...
    let currentblock = match provider.get_block_number().await {
        ...
        Err(_) => match provider.get_block_number().await {
            ...
            Err(_) => {
                firstmessage
                    .edit(ctx, |b| {
                        b.content("Couldn't get the latest block").ephemeral(true)
                    })
                    .await?;
                return Ok(());
            }
    ...
    let filter = Filter::new()
        ...
        .topic0(
            "0xf70d5c697de7ea828df48e5c4573cb2194c659f1901f70110c52b066dcf50826".parse::<H256>()?,
        )
    ...
    if bribevec.len() < 25 {
        firstmessage
            .edit(ctx, |b| b.embed(|c| c.fields(bribevec)))
            .await?;
    } else {
        ...
        firstmessage
            .edit(ctx, |b| {
                b.embed(|c| c.fields(tempvec3)).components(|a| {
                    a.create_action_row(|f| {
                        f.create_button(|d| {
                            d.custom_id("more")
                                .label("View more!")
            ...
            .await
        {
            for chunkss in tempvec.clone() {
                ...
                ctx.send(|b| { b.embed(|b| b.fields(chunk1)) }.ephemeral(true))
                    .await?;
